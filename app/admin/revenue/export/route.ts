...
function buildMonthlyRows(monthly: RevenueMonthlyRow[]): Array<Array<string | number>> {
  return monthly.map((month) => [
    month.month,
    month.revenue,
    month.offlineRevenue,
    month.onlineRevenue,
    month.offlineSoldCount,
    month.onlineSoldCount,
    month.soldCount,
    month.averagePrice,
...
function buildReportCsv(analytics: RevenueAnalytics): string {
  ...
  const rows: Array<Array<string | number>> = [
    ...
    ['집계 기준', 'artwork_sales AND sold_at IS NOT NULL'],
    ...
    ['manual 매출(원)', analytics.summaryBySource.manual.revenue],
    ...
    ['온라인 수량(점)', analytics.summaryByChannel.online.soldCount],
    ['온라인 매출 비중(%)', formatRate(analytics.summaryByChannel.onlineRevenueSharePct)],
...
function buildAccountingTemplateCsv(analytics: RevenueAnalytics): string {
  ...
  analytics.entries.forEach((entry, index) => {
    ...
    rows.push([
      ...
      soldAtKst,
      entry.soldAtUtc,
      'artwork_sales and sold_at not null',
    ]);
  });

export default function ArtworkEditLoading() {
  return (
    <div
      aria-hidden="true"
      className="mx-auto max-w-4xl space-y-6 rounded-2xl border border-gray-200 bg-white/90 p-6 shadow-sm sm:p-8"
    >
      {/* 이미지 업로드 행 */}
      <div className="space-y-2">
        ...
        <div className="flex gap-3">
          {[...Array(3)].map((_, i) => (
            <div key={i} className="h-20 w-20 flex-shrink-0 animate-pulse rounded-lg bg-gray-200" />
          ))}
        </div>
      </div>
      {/* 필드 8개 2열 */}

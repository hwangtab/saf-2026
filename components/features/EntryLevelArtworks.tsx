 ...
 * 8점 그리드 (모바일 2열·태블릿 3열·데스크탑 4열).
 *
 * **server-rendered card 패턴 (hydration 0)** — ArtworkCategoryGrid InlineGridCard와 동일 접근.
 * 'use client' ArtworkCard 8개 hydration이 모바일 4× CPU throttle에서 TBT 200ms+ 회귀 위험이 있어,
 * server card로 직접 inline. 메모리 feedback_hero_server_island_regression.md 원칙 부합.
 */
      ...
      <div className="relative w-full overflow-hidden aspect-[4/5] bg-canvas-soft">
        <div className="absolute inset-3 md:inset-4">
          <SafeImage
            ...
            loading="lazy"
            quality={70}
            sizes="(max-width: 640px) calc(50vw - 1.5rem), (max-width: 1024px) calc(33vw - 1.5rem), 280px"

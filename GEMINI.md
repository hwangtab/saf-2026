...
CSV에서 데이터를 가져올 때 다음 포맷팅 규칙을 반드시 적용하여 가독성을 높이고 불필요한 공백을 제거해야 합니다. **반드시 `npm run format-artworks` 명령어를 사용하여 이 규칙을 적용하세요.**

1.  **스마트 줄바꿈 (Smart Spacing)**:
    - **리스트 단일 줄바꿈**: 경력, 전시 목록 등 리스트 형태의 데이터는 **한 줄 간격**(`\n`)으로 붙여씁니다. (과도한 엔터 제거)
    - **섹션 분리 이중 줄바꿈**: 주요 섹션 헤더(`학력`, `개인전`, `단체전`, `수상`, `소장처` 등) 앞에는 **두 줄 간격**(`\n\n`)을 두어 구분을 명확히 합니다.
2.  **줄바꿈 축소**: 3개 이상의 연속된 줄바꿈(`\n\n\n+`)은 무조건 **2개**로 줄입니다.
3.  **공백 제거**: 각 줄의 앞뒤 공백(Trailing/Leading whitespace)을 제거합니다. (`trim()`)
4.  **탭 제거**: 탭 문자(`\t`)는 제거하거나 공백으로 변환합니다.
5.  **전체 트림**: 텍스트 전체의 시작과 끝에 있는 공백을 제거합니다.

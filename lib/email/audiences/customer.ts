...
export class CustomerAudienceResolver implements AudienceResolver {
  async resolve(): Promise<Recipient[]> {
    ...
    const { data: consentUsers } = await fetchAllInBatches<{
      ...
      console.error('[customer-audience] consent query error:', err);
    ...
    const { data: recentBuyers } = await fetchAllInBatches<{
      ...
      console.error('[customer-audience] buyer query error:', err);
    ...
    const { data: suppressions } = await fetchAllInBatches<{ email_hash: string }>((from, to) =>
      ...
      console.error('[customer-audience] suppressions query error:', err);

      ...
      try {
        ...
        for (const step of steps) {
          ...
          if (currentStats.size <= TARGET_SIZE_BYTES) {
            console.log(
              `  -> [SUCCESS] Resized to ${step.width}px, Q${step.quality}. New Size: ${formatBytes(currentStats.size)}`
            );
            break;
          }
        }
        ...
        if (finalStats.size > TARGET_SIZE_BYTES) {
          console.log(
            `  -> [WARN] Could not get under 1MB. Final Size: ${formatBytes(finalStats.size)}`
          );
        }
        optimizedCount++;
      } catch (error) {

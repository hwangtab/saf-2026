...
const headers = [
    ...
    '적립금', '적립금 구분', '공통이벤트 정보', '성인인증', '옵션사용', '품목 구성방식', '옵션 표시방식', '옵션세트명',
    '옵션입력', '옵션 스타일', '버튼이미지 설정', '색상 설정', '필수여부', '품절표시 문구', '추가입력옵션', '추가입력옵션 명칭',
    '추가입력옵션 선택/필수여부', '입력글자수(자)', '이미지등록(상세)', '이미지등록(목록)', '이미지등록(작은목록)', '이미지등록(축소)',
    '이미지등록(추가)', '제조사', '공급사', '브랜드', '트렌드', '자체분류 코드', '제조일자', '출시일자', '유효기간 사용여부',
    '유효기간', '원산지', '상품부피(cm)', '상품결제안내', '상품배송안내', '교환/반품안내', '서비스문의/안내', '배송정보',
    '배송방법', '국내/해외배송', '배송지역', '배송비 선결제 설정', '배송기간', '배송비 구분', '배송비입력', '스토어픽업 설정',
    '상품 전체중량(kg)', 'HS코드', '상품 구분(해외통관)', '상품소재', '영문 상품소재(해외통관)', '옷감(해외통관)',
...
function parsePrice(priceStr) {
    ...
    return numMatch ? { value: parseInt(numMatch[0], 10), isInquiry: false } : { value: 0, isInquiry: true };
}

// Classify genre
function classifyGenre(material) {
    const m = (material || '').toLowerCase();
    if (m.includes('oil on canvas') || m.includes('acrylic on canvas') || m.includes('캔버스에')) return '회화';
    if (m.includes('pigment print') || m.includes('archival') || m.includes('inkjet')) return '사진';
    if (m.includes('목판') || m.includes('etching') || m.includes('lithography') || m.includes('drypoint')) return '판화';
...
function generateHtmlDescription(artwork) {
    let html = '<div style="font-family: \'Noto Sans KR\', sans-serif; line-height: 1.8; color: #333;">';
    ...
    if (artwork.description) {
        html += '<div style="margin-bottom: 30px;">';
        html += '<h3 style="font-size: 18px; border-bottom: 1px solid #ddd; padding-bottom: 10px; margin-bottom: 15px;">작품 설명</h3>';
    ...
    if (artwork.profile) {
        html += '<div style="margin-bottom: 30px;">';
        html += '<h3 style="font-size: 18px; border-bottom: 1px solid #ddd; padding-bottom: 10px; margin-bottom: 15px;">작가 소개</h3>';
    ...
    if (artwork.history) {
        html += '<div style="margin-bottom: 30px;">';
        html += '<h3 style="font-size: 18px; border-bottom: 1px solid #ddd; padding-bottom: 10px; margin-bottom: 15px;">작가 이력</h3>';

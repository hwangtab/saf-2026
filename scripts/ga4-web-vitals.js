...
async function main() {
  ...
  if (!overall.rows || overall.rows.length === 0) {
    ...
    console.log('  가능한 원인:');
    console.log('  - 배포 후 측정 시작 전');
    console.log('  - 사용자 트래픽이 아직 없음 (5~10분 대기)');
    console.log('  - GA4 보고 처리 지연 (일반 보고서는 24~48시간 지연)\n');
  ...
  overall.rows.forEach((row) => {
    ...
    const display = isCls ? `${(Number(avg) / 1000).toFixed(3)} avg` : `${avg}ms avg`;
  ...
  if (byRating.rows && byRating.rows.length > 0) {
    ...
    byRating.rows.forEach((row) => {
      ...
      if (!grouped[metric])
        grouped[metric] = { good: 0, 'needs-improvement': 0, poor: 0, total: 0 };
    ...
    Object.entries(grouped).forEach(([metric, dist]) => {
      ...
      const niPct =
        dist.total > 0 ? ((dist['needs-improvement'] / dist.total) * 100).toFixed(1) : '0.0';
  ...
  if (byPageLcp.rows && byPageLcp.rows.length > 0) {
    console.log('=== LCP 가장 나쁜 페이지 Top 10 ===');
    byPageLcp.rows.forEach((row) => {
      ...
      console.log(
        `  ${rating}  avg=${String(avg).padStart(5)}ms  count=${String(count).padStart(3)}  ${pagePath}`
      );
    });
    console.log();
  }
  ...
  const byDevice = await runReport({
    dateRanges: [{ startDate, endDate: 'today' }],
    dimensions: [{ name: 'deviceCategory' }, { name: 'customEvent:metric_name' }],
    metrics: [{ name: 'eventCount' }, { name: 'eventValue' }],
    dimensionFilter: {
      filter: { fieldName: 'eventName', stringFilter: { value: 'web_vitals' } },
    },
  ...
  if (byDevice.rows && byDevice.rows.length > 0) {
    ...
    byDevice.rows.forEach((row) => {
      ...
      console.log(
        `  ${device.padEnd(8)} ${metric.padEnd(6)} avg=${String(avg).padStart(8)}  count=${count}`
      );
    });
    console.log();
  }

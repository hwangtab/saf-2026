...
function parseBatchFile(content) {
    ...
    while ((match = objectRegex.exec(content)) !== null) {
        ...
        if (id) {
            artworks.push({ id, artist, title, description, size, material, year, edition, price, image });
        }
    }
    return artworks;
}
...
function getGenre(material, title) {
    ...
    if (material.includes('canvas') || material.includes('유채') || material.includes('아크릴') || material.includes('수묵')) return '회화';
    if (material.includes('paper') && material.includes('print')) return '사진';
    if (material.includes('목판') || material.includes('실크스크린') || material.includes('Print')) return '판화';
    if (material.includes('resin') || material.includes('조각')) return '조각';
    if (material.includes('한지') && (material.includes('채색') || material.includes('먹'))) return '회화'; // or 민화 based on artist
    return '회화'; // default
}
...
function generateHTML(artwork, artistProfile) {
    ...
    return `<div style="font-family: 'Noto Sans KR', sans-serif; line-height: 1.8; color: #333;">` +

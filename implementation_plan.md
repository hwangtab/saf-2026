# 매거진 연관 링크/태그 정합성 개선 실행 계획 (2026-04-09)

## 1) 목적

- 매거진 상세 하단의 `작품 보기` / `관련 매거진` 노출 방식을 본문 수동 Markdown 의존에서 공통 자동 렌더링으로 통일한다.
- `stories.tags`가 실제 운영 UI에서 조회/수정되지 않는 단절을 해소해, 작가 글과 작품/작가 페이지의 연관 링크가 일관되게 유지되도록 한다.
- 기존 공개 글 중 태그 누락 및 본문 수동 footer가 섞인 상태를 정리해, 이후 신규 글도 동일한 규칙으로 운영할 수 있게 한다.

## 2) 현재 확인된 문제

- 공개 스토리 54개 중 `artist-story`는 32개다.
- 이 중 `tags`가 있는 글은 24개, 없는 글은 8개다.
- 본문 끝에 수동 `작품 보기 · 관련 매거진` 링크가 들어간 글은 5개뿐이다.
- 스토리 상세 페이지의 관련 작품 계산은 `story.tags[0]`에 의존한다.
- 관리자 매거진 UI와 저장 액션은 `tags`를 조회/편집/저장하지 않는다.
- 결과적으로 어떤 글은 본문 수동 링크가 있고, 어떤 글은 자동 관련 카드만 있고, 어떤 글은 둘 다 없어 보이는 상태가 발생한다.

## 3) 목표 상태

- 매거진 상세 하단에 공통 footer/CTA 영역을 두고, 본문 수동 링크 없이도 일관된 내부 링크를 제공한다.
- `artist-story`는 작가 태그 기준으로 `OOO의 작품 보기`와 `관련 매거진`을 자동 노출한다.
- 다른 카테고리는 카테고리 성격에 맞는 일반 링크만 노출하거나, 작가 연결이 없으면 작가 전용 CTA를 숨긴다.
- 관리자 콘텐츠 화면에서 `tags`를 직접 확인하고 수정할 수 있다.
- 기존 공개 글의 누락 태그와 수동 footer 본문을 정리해 중복/누락이 없도록 맞춘다.
...
- `app/[locale]/stories/[slug]/page.tsx`
  - 본문 아래 공통 footer/CTA 영역 추가
  - `artist-story` + 작가 태그 존재 시 작가 페이지 링크와 카테고리 링크 자동 노출
  - 태그가 없으면 일반 매거진/작품 링크만 제한적으로 노출하거나 숨김 처리

### B. 운영 UI 복구

- `app/(portal)/admin/content/stories/page.tsx`
- `app/(portal)/admin/content/stories/stories-manager.tsx`
- `app/actions/admin-content.ts`
  - `tags` 조회/표시/수정/저장 경로 추가
  - 운영자가 쉼표 기준으로 태그를 편집할 수 있게 정리

### C. 기존 데이터 정리

- 태그가 없는 `artist-story` 8건에 작가명 태그를 백필한다.
- 본문에 수동 footer가 들어간 글은 해당 줄을 제거해 공통 자동 footer와 중복되지 않게 정리한다.
- 기존 `tags[0]`가 실제 작가명과 정확히 매칭되는지 재확인한다.

## 5) 구현 절차

1. 관리자 매거진 CRUD에 `tags` 필드 노출/저장 추가
2. 스토리 상세용 공통 footer 렌더링 규칙 구현
3. 공개 스토리 중 누락 태그 8건 백필
4. 수동 footer가 들어간 본문 4건 정리
5. 작품 상세/작가 페이지의 관련 매거진 노출이 백필 결과를 따라 정상 연결되는지 확인
6. `walkthrough.md`에 변경 사항과 검증 결과 기록

## 6) 검증 계획

1. `artist-story` 글에서:
   - 자동 footer가 공통 위치에 보이는지
   - 작가 링크가 해당 작가 페이지로 정확히 이동하는지
2. 태그를 수정한 뒤:
   - 작품 상세 페이지 관련 매거진
   - 작가 페이지 관련 매거진
     가 함께 반영되는지 확인
3. 수동 footer 제거 후 본문 말미가 어색하지 않은지 확인
4. `npm run lint`
5. `npm run type-check`

## 7) 리스크 및 대응

- 작가명 표기가 DB 작가명과 다르면 자동 연결이 실패할 수 있다.
  - 대응: 백필 시 실제 `artists.name_ko` 기준으로 맞춘다.
- 운영 UI에 `tags`를 그대로 배열로 노출하면 편집성이 떨어질 수 있다.
  - 대응: 우선 쉼표 구분 문자열 입력으로 단순화하고, 저장 시 trim/빈값 제거를 적용한다.
- 본문 수동 footer 제거 시 문단 흐름이 깨질 수 있다.
  - 대응: 제거 대상 글의 본문 말미를 개별 확인하고 불필요한 공백/구분선도 함께 정리한다.

## 8) 완료 기준

- 매거진 상세 하단 링크가 본문 수동 삽입이 아니라 공통 로직으로 통일된다.
- 관리자 UI에서 `tags`를 조회/수정할 수 있다.
- 태그 누락 8건과 수동 footer 삽입 글이 정리된다.
- `npm run lint`, `npm run type-check`를 통과하고 결과가 `walkthrough.md`에 기록된다.

...
8. `wiki/index.md`의 해당 행 상태·요약을 갱신한다. **신규 페이지를 만들었으면 index.md에 새 행을 추가한다.**
...
2. 다음을 점검해 **우선순위 리포트**만 낸다(파일 변경 없음):
   ...
   - **고아**: `index.md` 카탈로그에도 없고 어디서도 `[[링크]]`로도 닿지 않는 페이지 / 끊긴 링크(없는 페이지 가리킴). index.md 등재 페이지는 고아로 판정하지 않는다.

...
3. **신뢰도 가드** — `ratio = max(w,h)/min(w,h)`:
   ...
   - `ratio ≤ 2.2` (정사각 S형 포함) → `confident=true`, `약 N호` 표기. 정사각은 S 규격표 대신 긴 변을 F형 표에 매칭한 근사값(운영 결정: 정사각도 호수 표기, S 규격표만 미사용). 실측상 정사각 작품이 2D의 ~16%(66/426)로 적지 않아 생략 시 호수 표기가 대량 누락됨.

...
경계는 **호수 사이 중간값**으로 둔다 — `small/medium`=10호(2412)·12호(3030) 중간, `medium/large`=30호(6608)·40호(8030) 중간, `large/xlarge`=100호(21135)·120호(25265) 중간. 이렇게 해야 라벨("~10호"/"10–30호"/"30–100호"/"100호~")이 호수와 정확히 일치(예: 30호 작품 90.9×72.7=6608.43이 large로 새지 않고 중형에 포함).

| bucket   | 조건                  | 호수 의미 |
| -------- | --------------------- | --------- |
| `object` | depth != null (3D)    | 입체      |
| `small`  | area ≤ 2,721          | ~10호     |
| `medium` | 2,721 < area ≤ 7,319  | 10–30호   |
| `large`  | 7,319 < area ≤ 23,200 | 30–100호  |
| `xlarge` | area > 23,200         | 100호~    |

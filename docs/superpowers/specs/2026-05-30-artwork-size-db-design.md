...
| 구간           | 개수 | 비율 | 면적범위(cm²)  |
| -------------- | ---- | ---- | -------------- |
| 소품(~10호)    | 389  | 72%  | 81 – 2,700     |
| 중형(10–30호)  | 128  | 24%  | 2,736 – 7,215  |
| 대형(30–100호) | 22   | 4%   | 7,775 – 20,430 |
| 특대(100호~)   | 0    | 0%   | —              |

→ **특대(100호 초과)는 0건.** 통념(100호↑=전시급)상 자연스러움. **0건 구간은 필터 UI에서 동적 숨김**. (경계는 §4.4 — 호수 사이 중간값으로 두어 30호=중형 등 라벨 정합.)
...
// 구간 분류. depth 있으면 'object'. 호수(긴 변=estimateHo) 기반 4단계 — ho 표기와 동일 척도.
...
3. **신뢰도 가드** — `ratio = max(w,h)/min(w,h)`:
   ...
   - `ratio ≤ 2.2` (정사각 S형 포함) → `confident=true`, `약 N호` 표기. **S형(정사각)의 한 변 = 같은 호수 F형의 긴 변**(무사시노미대 표준 정의)이므로, 긴 변을 F형 표에 매칭하는 것이 S형 규격표를 직접 쓰는 것과 **수학적으로 동일**하다(근사 아님) — 별도 S 표 없이 정사각이 정확히 S호수로 환산된다(예: 45.5×45.5 → S8 = 8호). 실측상 정사각이 2D의 ~16%(66/426)라 생략 시 호수 표기가 대량 누락됨.
   - `HO_TABLE`은 0호(18×14, 긴변 18.0)부터 — 그 미만 소품이 1호로 과대 표기되던 블라인드 스팟 보정.
4. placeholder/파싱 실패 → `null`.

### 4.4 구간 경계 (호수=긴 변 기준)

구간은 **`estimateHo`가 낸 호수**로 분류한다 — `cm · 약 N호 · 구간`이 한 줄에 같이 노출되므로(상세 페이지) 호수와 구간이 **같은 척도**여야 어긋나지 않는다. 과거 면적 기준은 정사각(면적↑로 한 칸 위)·좁은 직사각(면적↓로 한 칸 아래)에서 호수와 어긋나, 공개 평면작 397점 중 33점(8.3%)이 "약 10호 · 중형"처럼 모순 표기됐다 → 호수로 통일해 0건.

백필 SQL은 컬럼 함수 없이 돌도록 **긴 변 cm 중간점**으로 동일 결과를 낸다(인접 호수 긴 변 평균): 10\|12호=56.8, 30\|40호=95.45, 100\|120호=178.05.

| bucket   | 조건(호수)        | 백필 SQL(긴 변 cm)     |
| -------- | ----------------- | ---------------------- |
| `object` | depth != null(3D) | depth_cm is not null   |
| `small`  | ho ≤ 10           | 긴 변 ≤ 56.8           |
| `medium` | 10 < ho ≤ 30      | 56.8 < 긴 변 ≤ 95.45   |
| `large`  | 30 < ho ≤ 100     | 95.45 < 긴 변 ≤ 178.05 |
| `xlarge` | ho > 100          | 긴 변 > 178.05         |

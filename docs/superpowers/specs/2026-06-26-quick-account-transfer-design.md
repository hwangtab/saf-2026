...
- `createBankTransferOrder` (`app/actions/checkout.ts`) — 호출 제거. 함수 본체도 제거
  (다른 호출자 없음 확인 후).
- `register_event_bank_transfer` 호출 경로 (`event-registration.ts`의 transfer 분기).
- 관리자 "입금 확인" UI/버튼(`confirm_bank_transfer_order` RPC 호출부) — 무통장 주문이 더는
  생성되지 않으므로 UI 진입점 제거.
- `metadata.payment_provider='manual_bank_transfer'` 생성 로직, 계좌 안내 i18n 키, 무통장
  관련 안내 컴포넌트.

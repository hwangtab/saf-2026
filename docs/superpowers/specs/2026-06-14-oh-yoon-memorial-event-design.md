...
- 정원 초과 시 대기자(무료) 등록
- 알림톡(SMS) 확인 발송 (신청확인·대기등록·대기자 결제안내)
...
- 알림톡 3종 (기존 Solapi 인프라 재사용):
  1. 신청확인 (결제완료)
  2. 대기등록 완료
  3. 대기자 결제안내 (자리 발생 시)
- 신규 알림톡 템플릿은 카카오 심사가 필요(기존 사례 7종 심사통과 이력). 심사 대기 기간에는 승인된 일반 SMS fallback 또는 이메일 병행. env 미설정 시 no-op (기존 정책).
...
- 카카오 알림톡 신규 템플릿 심사 일정 (행사일 7/5 전 심사 통과 필요 — 심사 기간 고려해 조기 신청)
- 대기자 자동 승격은 v2 후보

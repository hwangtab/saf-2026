...
- `/Users/hwang-gyeongha/saf/app/(portal)/admin/content/stories/page.tsx`
  - 매거진 관리자 목록 조회에 `tags` 컬럼을 추가

- `/Users/hwang-gyeongha/saf/app/(portal)/admin/content/stories/stories-manager.tsx`
  - 매거진 생성/수정 폼에 `tags` 입력 추가
  - 쉼표 구분 문자열을 배열로 바꾸는 클라이언트 보조 로직 추가
  - 검색 대상에 태그를 포함

- `/Users/hwang-gyeongha/saf/app/actions/admin-content.ts`
  - 스토리 생성/수정 액션에서 `tags`를 저장하도록 보강

- `/Users/hwang-gyeongha/saf/app/[locale]/stories/[slug]/page.tsx`
  - `artist-story`의 첫 유효 태그를 작가명으로 해석
  - 하단 내부 링크를 본문 수동 Markdown 대신 공통 자동 footer로 통일
  - 작가 태그가 있을 때만 `{작가명}의 작품 보기` 링크를 노출
  - `artist-story`에서 태그가 없을 때는 무작위 작품 폴백을 하지 않도록 조정

- `/Users/hwang-gyeongha/saf/supabase/migrations/20260409153000_normalize_story_tags_and_footer_links.sql` (신규)
  - 작가 글 7건에 누락된 작가 태그 백필
  - 본문 끝에 직접 삽입된 `작품 보기 · 관련 매거진` 수동 footer 4건 제거

## 데이터 반영 결과

- 공개 `artist-story` 32건 중 태그 보유 글이 `24건 → 31건`으로 증가
- 현재 태그가 없는 `artist-story`는 `127-artists-solidarity` 1건만 남음
  - 단일 작가 글이 아닌 묶음 소개 글이라 이번 백필 대상에서 제외
- 본문 수동 footer 형태의 작가 글은 `4건 → 0건`으로 정리
- buying guide 1건의 일반 갤러리 CTA 문구는 유지

## 검증 결과

- `npm run lint` 통과
- `npm run type-check` 통과
- 원격 Supabase에 동일 SQL 반영 후 재집계 확인
- 로컬 확인용 스크린샷 저장
  - `/Users/hwang-gyeongha/saf/output/playwright/story-footer-kim-dongseok.png`
...
## 작품 상세 작가 관련 URL 확장 (1차 배치)

### 반영 내용

- `/Users/hwang-gyeongha/saf/content/artist-articles.ts`
  - `손장섭` 신규 등록: 연합뉴스 2건, 서울아트가이드 1건 추가
  - `김영서` 공백 해소: 서울문화포털 2건, 미디어펜, 청년타임스 자료 추가
  - `변경희` 공백 해소: 서울문화포털, 서울아트가이드, CICA Museum 자료 추가
  - `이채원` 공백 해소: 경기도미술관 전시 페이지, 아트인컬처 자료 추가
  - `칡뫼 김구` 키를 작품 데이터와 동일한 `'칡뫼 김구'`로 수정
  - `칡뫼 김구`에 서울아트가이드 전시 아카이브 1건 추가
...
- 제외:
  - 작가 개인 웹사이트
  - 작가 본인 SNS
  - 소속 갤러리/에이전시/플랫폼의 단순 프로필 페이지
- 포함:
  - 공공기관/미술관/문화재단 전시 페이지
  - 신뢰 가능한 언론 기사 및 평론
  - 전시 아카이브 성격의 제3자 자료
- 동명이인 검수:
  - 작품 매체, 전시명, 기관명, 약력 단서 중 2개 이상 일치한 경우만 반영
- 중복 방지:
  - 신규 추가 후 `content/artist-articles.ts` 내 exact URL 기준 중복 0건 확인

### 보류 작가

- `남진현`
  - 기사 후보는 있었지만 공공기관/제3자 자료가 충분히 쌓이지 않아 보류
- `림지언`
  - 검색 결과가 타 분야 동명이인과 강하게 섞여 보류
- `박영선`
  - 동명이인 리스크가 높아 추가 단서 확보 전 보류
- `이지은`
  - 독일 활동 이력과 연결되는 제3자 자료는 있었지만 공개 상세 페이지에 넣기에는 검증 밀도가 부족해 보류
- `김유진`
  - 플랫폼/프로필성 페이지 비중이 높아 이번 배치에서는 제외
- `작가미상`
  - 작가 식별 불가로 수집 대상 제외
...
- `npx eslint content/artist-articles.ts` 통과
- `npm run type-check` 통과
...
- `/Users/hwang-gyeongha/saf/components/common/Header/DesktopNav.tsx`
  ...
  - `검색`, `주문/배송 조회`는 기존 버튼형 인지성을 유지한 채 남기고 중복 CTA만 제거
  - 관리자/마이페이지 진입 버튼도 버튼형 표현을 유지해 접근성을 보존
  - 언어 전환은 기존 위치를 유지
- `/Users/hwang-gyeongha/saf/components/common/Header/FullscreenMenu.tsx`
  - 모바일에서 메인 탐색과 보조 유틸 영역을 분리
  - `주문/배송 조회`, 관리자/마이페이지 진입을 별도 보조 섹션으로 이동하되 버튼형 인지성 유지
  - 하단 CTA는 `조합원 가입`만 유지
...
- 공개 헤더에서 중복 CTA만 제거하고 기존 유틸 버튼의 사용성은 유지
- 관리자/마이페이지와 주문조회가 다시 버튼으로 읽혀 진입성이 좋아짐

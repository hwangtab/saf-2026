...
- **출품 작가 ≠ 금융 피해 당사자**. 씨앗페 출품 작가들은 동료 예술인의 금융 차별 문제를 해결하기 위해 **자발적으로 참여한 연대자**들이다. 이들을 "대출 못 받는 불우한 작가"로 프레이밍하면 안 됨. (실 출품 작가 수는 `lib/site-stats.ts`의 `ARTIST_COUNT` 동적 변수 — 콘텐츠/카피에 숫자 직접 박지 말고 i18n ICU `{artistCount}` 변수로 참조).
